...
extern void setFrequency(double pFrequency);
extern int fd;
...
private:
    ...
    Xrt_GetSignalInfo(rtwCAPI_ModelMappingInfo* mmi);
    Xrt_GetParameterInfo(void *rtw_mpi);
    // ----- Log Variables -----
    double* logvars;
...
int ZenomMatlab::initialize()
{
    ...
    contvars = new double[nBlockParams];
    logvars = new double[nSignals];
    ...
    for(paramIdx = 0; paramIdx < rtwCAPI_GetNumBlockParameters(mmi); paramIdx++){
        ...
        char* paramName = concatParamName(xrtpi[paramIdx].blockName, xrtpi[paramIdx].paramName );
        ...
        contvars[paramIdx] = xrtpi[paramIdx].dataValue;
    }
    
    for (sigIdx = 0; sigIdx < rtwCAPI_GetNumSignals(mmi); ++sigIdx){

        char* sigName = concatParamName(xtsi[sigIdx].blockName, xtsi[sigIdx].signalName );
        ...
        logvars[paramIdx] = xrtpi[paramIdx].dataValue;
    }
...
char* ZenomMatlab::concatParamName(char * blockName, char * paramName){
    ...
    char* name = malloc( sizeof(char)*MAX_NAMES_SIZE );
    ...
    case rtcase: \
        ...
        if (xtsi[sigIdx].isComplex==0) { \
            ...
            switch ( xrtpi[sigIdx].dataOrientation) { \
                case rtwCAPI_SCALAR: {\
                    xtsi[sigIdx].dataValue[0] = (double)*param;}\
                    break; \
                case rtwCAPI_VECTOR: \
                    for (colIdx = 0; colIdx < xtsi[sigIdx].numRows*xtsi[sigIdx].numColumns; colIdx++) { \
                        xtsi[sigIdx].dataValue[colIdx] = (double)param[colIdx]; \
                        \
                    } \
                    break; \
                case rtwCAPI_MATRIX_ROW_MAJOR: \
                    for(rowIdx = 0; rowIdx < xtsi[paramIdx].numRows; rowIdx++){ \
                        for(colIdx = 0; colIdx < xtsi[paramIdx].numColumns; colIdx++) { \
                            xtsi[paramIdx].dataValue[rowIdx*xtsi[paramIdx].numColumns + colIdx] = param[rowIdx*xtsi[paramIdx].numColumns+colIdx]; \
                        } \
                    } \
                    break; \
                case rtwCAPI_MATRIX_COL_MAJOR: \
                    for(rowIdx = 0; rowIdx <  xtsi[paramIdx].numRows; rowIdx++)  { \
                         for(colIdx = 0; colIdx < xtsi[paramIdx].numColumns; colIdx++){ \
                            xtsi[paramIdx].dataValue[colIdx*xtsi[paramIdx].numRows + rowIdx] = param[colIdx*xtsi[paramIdx].numRows+ rowIdx]; \
                        } \
                    } \
                    break; \
...
int ZenomMatlab::Xrt_GetSignalInfo(rtwCAPI_ModelMappingInfo* mmi){
    ...
    for(sigIdx=0; sigIdx < nSignals; sigIdx++){
        ...
        addrIdx     = rtwCAPI_GetBlockParameterAddrIdx(blockParams, sigIdx);
